...
"""
    Group Chat Robot v0.1 in python
...
@itchat.msg_register([TEXT], isFriendChat=True)  # isFriendChat   isGroupChat
def text_reply(msg):

    if msg['User']['NickName'] == '矢之':  # 'AA在路上机票 小侯18401733656':
        print('Message from: %s' % msg['User']['NickName'])
        # 发送者的昵称
        username = msg['FromUserName']:  # msg['ActualNickName']   # ????
...
itchat.auto_login(enableCmdQR=True, hotReload=True)   # 在cmd上二维码显示正常
itchat.run()
